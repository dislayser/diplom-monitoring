...
function ui_input($name, $lable = "", $parent = "", $id = "", $value = "", $type = "text"){
    $ui = '';
    ...
    //Input
    if (!empty($name)){
        $block_input .= '<input type="'.$type.'" class="form-control" name="'.$name.'" id="'.$id.'" value="" required>';
    } 
...
function ui_submit($name = null, $text = null, $class = 'btn btn-primary', $attr = ''){
    $ui = '';
    if(!empty($name) && !empty($text)){
        $ui .= '<button type="submit" class="'.$class.'" name="'.$name.'" id="'.$name.'" '.$attr.'>'.$text.'</button>';
    }
    return $ui;
}
